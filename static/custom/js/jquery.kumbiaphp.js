    ...
    $.KumbiaPHP = {
        ...
        cConfirm: function(event) {
            ...
            if(este.hasClass('js-link')) {
                ...
                footer.append('<a class="btn btn-success js-link js-spinner" href="'+data_to+'">'+gettext('ACCEPT')+'</a>');
            } else {
                footer.append('<button class="btn btn-success">'+gettext('ACCEPT')+'</a>');
            }
            
            footer.append('<button class="btn btn-default" data-dismiss="modal" aria-hidden="true">'+gettext('CANCEL')+'</button>');
        ...
        bindDatePicker: function() {
            ...
            // Carga el datetimepicker, para poder usar cache, fue quitado por sullon
            //$.ajax({ dataType: "script",cache: true, url: $.KumbiaPHP.publicPath + 'datepicker/bootstrap-datetimepicker.min.js'}).done(function() {                
            //    bindInputs();
            //});            
        ...
        initialize: function() {
            ...
            this.publicPath = src.substr(0, src.length - 29);
